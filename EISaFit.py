...
def load_data(files=None):
    ...
    if files:
        ...
        tpb = TopProgressBar(tasks=len(files))
...
def prefs():
    ...
    top.iconbitmap(r'group-30_116053.ico')
    top.grab_set()
    top.protocol("WM_DELETE_WINDOW", save_configs_and_close)
    ...
    var_mode = BooleanVar()
    var_mode.set(True)
    # Checkbutton(frm_spec, text='Analysis mode only', variable=var_mode, command=lambda: applyMode(var_mode.get())).grid(row=0, column=0, sticky='w')
    # Checkbutton(frm_spec, text='Disable ToolTips').grid(row=0, column=1, sticky='w')
    # Checkbutton(frm_spec, text='Configure default directory').grid(row=2, column=0, sticky='w')
