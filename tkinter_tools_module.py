...
def circuit_editor(func):
    """
    Decorator function.

    :param
    ---------------
    func: function from NewTabGUI.fitting()
    ...
    def wrapper(self):
        """
        wrapper function.
        Wrapper's the fitting function with a tkinter window to assign the circuit model and the initial guess.

        Display a new window that allows the user to input data

        Parameters
        -----------------
        self : class NewTabGUI
            since this wrapper is called from a method inside a class,
            it takes an positional argument, the instance of the class NewTabGUI
        """

        def build_entries(e=None, from_buf=False, values=None):
            """
            ...
            def apply_fit():
                """
                Gets the values for the fitting and calls the fitting function
                """
                b_fit['state'] = DISABLED
                # Making sure to get parameters

                # Getting the list of entries
                # path             para_frame              para_frame2          entries
                entries_list = top.winfo_children()[-1].winfo_children()[1].winfo_children()
                initial_guess = [float(entry.get()) for entry in entries_list]

                # Getting constants
                cons = {}
                for i, check in enumerate(checks):
                    if check.get():
                        cons[parameters[i]] = initial_guess[i]
                        initial_guess[i] = None
                circuit = circuit_entry.get()
                circuit = circuit.replace(' ', '')

                window = [int(lw_bound.get()), int(up_bound.get())]
                # Fitting function
                try:
                    if len(cons) == 0:
                        func(self=self,
                             ...
                             cons=None,
                             button=b_fit,  # to enable after fitting completed
                             window=window)
                    else:
                        func(self=self,
                             circuit=circuit,
                    ...
                    b_fit['state'] = NORMAL
                    build_entries()  # Update entries

            # Getting the para_frame
            top.winfo_children()[-1].destroy()

            para_frame = LabelFrame(top, text='Parameters')
            para_frame.pack()
            ...
            string = circuit_entry.get()
            string = string.replace(' ', '')
            circuit_entry.delete(0, 'end')
            circuit_entry.insert(0, string)
            ...
            for i, char in enumerate(string):
                ...
                if char == 'R':
                    ...
                    parameters.append(string[i:i + 2])
                    e = Entry(para_frame2)
                    e.pack(pady=3)
                ...
                if char == 'W':
                    if string[i:i+2] == 'Ws' or string[i:i+2] == 'Wo':
                        ...
                        parameters.append(f'{string[i:i + 3]}_0')
                        Label(para_frame1, text='tau/s').pack(pady=2)
                        parameters.append(f'{string[i:i + 3]}_1')
                        e = Entry(para_frame2)
                        e.pack(pady=3)
                        if not from_buf:
                            e.insert(0, '3000')
                ...
                if char == 'C':
                    if string[i:i + 3] == 'CPE':
                        ...
                        parameters.append(f'{string[i:i + 4]}_0')
                        e = Entry(para_frame2)
                        e.pack(pady=3)
                        if not from_buf:
                            e.insert(0, '0.000001')
                        ...
                        parameters.append(f'{string[i:i + 4]}_1')
                        e = Entry(para_frame2)
                        e.pack(pady=3)
                        if not from_buf:
                            e.insert(0, '0.9')
            ...
            b_fit = Button(para_frame, text='Apply Fitting',
                           command=lambda: threading.Thread(target=apply_fit).start(), bg='azure2')
            b_fit.pack(fill=BOTH, expand=1, side=BOTTOM)
        ...
        def build_from_buffer(initial=True):
            if initial:
                eq_circ, values = get_buffer(file_name='init_buf')
            else:
                eq_circ, values = get_buffer(file_name='fit_buf')

            circuit_entry.delete(0, END)
            circuit_entry.insert(0, eq_circ)
            build_entries(from_buf=True, values=values)

        top = Toplevel()
        top.resizable(0, 0)
        top.attributes('-topmost', 'true')
        top.transient()
        top.title(self.label)

        bound_frame = LabelFrame(top, text='Data Window (indexes)')
        lw_bound = Entry(bound_frame)
        lw_bound.pack(side=LEFT)
        lw_bound.insert(0, '0')
        up_bound = Entry(bound_frame)
        up_bound.pack(side=LEFT)
        up_bound.insert(0, '100')
        bound_frame.pack(pady=10)

        circ_frame = LabelFrame(top, text='Equivalent Circuit')
        circ_frame.pack(fill=BOTH, expand=1)
        inner_frame = Frame(circ_frame)
        inner_frame.pack()
        btn_last = Button(inner_frame, text='Last initial guess', bg='pale green', command=build_from_buffer)
        btn_last.pack(side=LEFT, fill=X)
        btn_last = Button(inner_frame, text='Last calculated fit', bg='bisque2',
                          command=lambda: build_from_buffer(initial=False))
        btn_last.pack(side=LEFT, fill=X)

        circuit_entry = Entry(circ_frame, width=30)
        circuit_entry.pack(side=LEFT, fill=BOTH, expand=1, padx=2)
        # circuit_entry.bind('<Any-KeyRelease>', build_entries)
        circuit_entry.insert(0, 'R0-p(R1-Wo1,CPE1)')

        btn = Button(inner_frame, text='Use Default', command=build_entries, bg='azure2')
        btn.pack(side=LEFT, fill=X)

        para_frame = LabelFrame(top, text='Parameters')

        build_from_buffer(initial=False)
...
class NewTabGUI:
    ...
    def __init__(self, master, label='New tab'):
        ...
        Button(head_frame, text="x", command=self.close_tab, width=2, height=1, bg="tomato2", bd=1).pack(side=RIGHT,
                                                                                                         anchor='n')
        Button(head_frame, text="Fitting", command=self.fitting).pack(side=LEFT)
        Button(head_frame, text="Table", command=self.show_table).pack(side=LEFT)
        Button(head_frame, text="Save", command=self.save).pack(side=LEFT)
        ...
        r1 = Radiobutton(head_frame, text='.xlsx', variable=self.extension, value='.xlsx', bg='white')
        r2 = Radiobutton(head_frame, text='.csv', variable=self.extension, value='.csv', bg='white')
        r1.pack(side=LEFT)
        r2.pack(side=LEFT)
        r1.select()
    ...
    def close_tab(self):
        answer = messagebox.askquestion(f'Close tab: {self.label}?', 'Are you sure you want to close this tab?'
                                                                     '\nIt is not possible to go back after this '
    ...
    @circuit_editor
    def fitting(self, circuit, initial_guess, cons, button, window=None):
        """
        ...
        # TODO check better this saving algorithm
        params_txt = circuit.circuit + '\n'
        init_txt = circuit.circuit + '\n'
        if cons is not None:
    ...
    def clear_fittings(self):
        # TODO close all parameters tops
