...
def load_data(file=None):
    ...
    try:
        ...
        Active_tab = my_notebook.add_costume_tab(label=file.split('/')[-1])
...
def start_runner():
    ...
    df.to_excel('smart_fit_teste.xlsx')
...
def start_coms(idx=None):
    ...
    if make_unique.get():
        ...
        Active_tab = my_notebook.add_costume_tab(label=f'{idx} Sensor')
...
runner_var = IntVar()
my_check = Checkbutton(main_frame, text='Smart Runner', variable=runner_var, command=disable_radios, bg='white')
my_check.pack()
...
my_menu.add_command(label='Fitting Editor', command=lambda: fit_editor.open())

Active_tab = my_notebook.add_costume_tab()
